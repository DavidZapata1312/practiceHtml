...
FORM_HTML = dedent("""
    ...
    <body>
        ...
        <form action="/tarjeta" method="get">
            ...
            <label>Edad: <input name="edad" type="number" required></label><br>
            <label>Profesion: <input name="profesion" required></label><br>
...
class TarjetaHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Si la ruta es "/tarjeta", generamos una tarjeta con los datos recibidos
            ...
            profesion = html.escape(datos.get("profesion",  ["Desempleado"])[0])           
            ...
            self._send_html(CARD_HTML.format(nombre=nombre, edad=edad,profesion=profesion,bio=bio, color=color))
        else:
